...
import type { AnalysisData } from "./AnalysisCharts"; // Ensure interface matches response
...
export default function Hero() {
  ...
  // ✅ State to store demographic_analysis data
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Correctly set the demographic_analysis data
      setAnalysisData(responseData.demographic_analysis ?? null);
    ...
    <section className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-[#E9E4F4] via-[#E9E4F4] to-[#FDECE6] text-center px-4">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            {imagePreviews.map((preview, index) => (
              <div key={index} className="relative w-24 h-24 rounded-lg overflow-hidden">
                <img src={preview} alt={`Uploaded Preview ${index + 1}`} className="w-full h-full object-cover" />
                <button
      ...
      {analysisData && (
        <div className="w-full mt-10">
          <h2 className="text-3xl font-bold text-purple-800 mb-6">Demographic Analysis</h2>
